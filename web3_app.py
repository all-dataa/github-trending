...
def get_ai_analysis(path):
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "你是一个 github trends 分析专家。负责分析 github 每日 rust 项目的趋势。将英文介绍翻译成中文。输出整齐精致。接着在下一行，安利一个最惊艳的项目。再换一行，最后总结今天的趋势项目关注的领域和特点。语言保持简洁。最后一句话：产品联系: pxxhl@qq.com"},
...
def job():
    ...
    while attempts < 6:
        try:
            ...
            scrape('rust', filename)   # weak point
            print('scape ends\n')
...
def daily_task():
    try:
        ...
        tos = get_emails('web3_emails.txt') 
        subject = '今日Web3+头条项目'
